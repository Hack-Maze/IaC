...
resource "local_file" "worker2_private_key" {
  ...
  resource "time_sleep" "wait_60_seconds_for_jump_ok" {
...
resource "null_resource" "transfer_pem" {
  ...
  depends_on = [time_sleep.wait_60_seconds_for_jump_ok]

  provisioner "file" {
    source      = "/tmp/.ssh/control_public_key.pub"
    destination = "/home/hackmaze-user/.ssh/control_public_key.pub"
    }
  ...
  provisioner "file" {
  source      = "/tmp/.ssh/worker1_public_key.pub"
  destination = "/home/hackmaze-user/.ssh/worker1_public_key.pub"
  }
  ...
  provisioner "file" {
  source      = "/tmp/.ssh/worker2_public_key.pub"
  destination = "/home/hackmaze-user/.ssh/worker2_public_key.pub"
  }
