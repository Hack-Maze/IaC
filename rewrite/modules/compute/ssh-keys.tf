...
resource "local_file" "worker2_private_key" {
  ...
  depends_on = [azurerm_linux_virtual_machine.jump_server,
    ...
    provisioner "local-exec" {
      command = "scp -i  /tmp/.ssh/jump_private_key.pem /tmp/.ssh/control_private_key.pem ec2-user@${var.control_static_private_ip}:/home/hackmaze-user/.ssh/"
    }

   provisioner "local-exec" {
      command = "scp -i  /tmp/.ssh/jump_private_key.pem /tmp/.ssh/worker1_private_key.pem ec2-user@${var.worker1_static_private_ip}:/home/hackmaze-user/.ssh/"
    }
